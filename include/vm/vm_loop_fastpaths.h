...
static inline bool vm_typed_iterator_bind_range(uint16_t reg,
                                                int64_t start,
                                                int64_t end,
                                                int64_t step) {
    if (reg >= REGISTER_COUNT || step == 0) {
        if (reg < REGISTER_COUNT) {
            vm.typed_iterators[reg].kind = TYPED_ITER_NONE;
        }
        return false;
...
static inline bool vm_typed_iterator_bind_array(uint16_t reg, ObjArray* array) {
    return vm_typed_iterator_bind_array_at(reg, array, 0u);
}
...
bool vm_exec_monotonic_inc_cmp_i64(uint16_t counter_reg, uint16_t limit_reg, bool* out_should_continue);
bool vm_exec_monotonic_inc_cmp_u32(uint16_t counter_reg, uint16_t limit_reg, bool* out_should_continue);
bool vm_exec_monotonic_inc_cmp_u64(uint16_t counter_reg, uint16_t limit_reg, bool* out_should_continue);
