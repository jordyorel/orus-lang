...
static bool test_mul_i32_imm_reuses_typed_cache(void) {
    ...
    if (!vm.typed_regs.dirty[0]) {
        fprintf(stderr, "Expected register 0 to remain dirty after repeated multiplies\n");
        success = false;
        goto cleanup;
    }
    ...
    if (!IS_I32(vm.registers[0]) || AS_I32(vm.registers[0]) != 2) {
        fprintf(stderr, "Expected boxed register to remain stale (2), got type %d value %d\n",
                vm.registers[0].type,
