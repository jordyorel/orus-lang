...
void initMultiPassCompiler(Compiler* compiler, Chunk* chunk,
    ...
    printf("[DEBUG] Temporarily skipping lifetime analyzer initialization\n");
    
    compiler->lifetimeAnalyzer = NULL;
    // compiler->lifetimeAnalyzer = malloc(sizeof(LifetimeAnalyzer));
    // if (compiler->lifetimeAnalyzer) {
    //     printf("[DEBUG] About to call initLifetimeAnalyzer\n");
    //     
    //     initLifetimeAnalyzer((LifetimeAnalyzer*)compiler->lifetimeAnalyzer);
    //     printf("[DEBUG] initLifetimeAnalyzer completed\n");
    //     
...
void initIterativeContext(IterativeContext* ctx) {
    printf("[DEBUG] initIterativeContext: Starting\n");
    
    if (!ctx) return;
    
    printf("[DEBUG] initIterativeContext: About to initialize work queue\n");
    
    // Initialize work queue using proper memory management
    ctx->workQueueCapacity = 0;  // Start with 0, will grow as needed
    ctx->workQueue = NULL;
    ctx->workQueueSize = 0;
    printf("[DEBUG] initIterativeContext: Work queue initialized\n");
    
    
    // Initialize scope stack using proper memory management
    ctx->scopeStackCapacity = 0;  // Start with 0, will grow as needed
    ctx->scopeStack = NULL;
    ctx->scopeDepth = 0;
    ctx->maxScopeDepth = 0;
    
    // Initialize jump patching using proper memory management
    ctx->jumpCapacity = 0;  // Start with 0, will grow as needed
    ...
    printf("[DEBUG] initIterativeContext: All initialization complete\n");
...
void cleanupIterativeContext(IterativeContext* ctx) {
    ...
    printf("[DEBUG] cleanupIterativeContext: Starting cleanup\n");
    
    // Cleanup work queue using proper memory management
    printf("[DEBUG] cleanupIterativeContext: About to cleanup work queue\n");
    if (ctx->workQueue) {
        FREE_ARRAY(WorkItem, ctx->workQueue, ctx->workQueueCapacity);
        ctx->workQueue = NULL;
    }
    printf("[DEBUG] cleanupIterativeContext: Work queue cleaned up\n");
    
    // Cleanup scope stack (and nested allocations)
    printf("[DEBUG] cleanupIterativeContext: About to cleanup scope stack\n");
    if (ctx->scopeStack) {
        for (int i = 0; i < ctx->scopeDepth; i++) {
            printf("[DEBUG] cleanupIterativeContext: Cleaning scope frame %d\n", i);
            ScopeFrame* frame = &ctx->scopeStack[i];
            if (frame->breakJumps) FREE_ARRAY(JumpPatch, frame->breakJumps, frame->breakCapacity);
            if (frame->continueJumps) FREE_ARRAY(JumpPatch, frame->continueJumps, frame->continueCapacity);
        }
        FREE_ARRAY(ScopeFrame, ctx->scopeStack, ctx->scopeStackCapacity);
        ctx->scopeStack = NULL;
    }
    printf("[DEBUG] cleanupIterativeContext: Scope stack cleaned up\n");
    
    // Cleanup jump patches using proper memory management
    printf("[DEBUG] cleanupIterativeContext: About to cleanup pending jumps\n");
    if (ctx->pendingJumps) {
        FREE_ARRAY(JumpPatch, ctx->pendingJumps, ctx->jumpCapacity);
        ctx->pendingJumps = NULL;
    }
    printf("[DEBUG] cleanupIterativeContext: Pending jumps cleaned up\n");
...
static void expandWorkQueue(IterativeContext* ctx) {
    int oldCapacity = ctx->workQueueCapacity;
    ctx->workQueueCapacity = GROW_CAPACITY(oldCapacity);
    ctx->workQueue = GROW_ARRAY(WorkItem, ctx->workQueue, oldCapacity, ctx->workQueueCapacity);
}
...
bool hasWork(IterativeContext* ctx) {
    printf("[DEBUG] hasWork: ENTRY - ctx: %p\n", ctx);
    if (!ctx) {
        printf("[DEBUG] hasWork: ctx is NULL, returning false\n");
        return false;
    }
    
    printf("[DEBUG] hasWork: ctx->workQueueSize: %d\n", ctx->workQueueSize);
    bool result = ctx->workQueueSize > 0;
    
    if (!result) {
        printf("[DEBUG] hasWork: about to return false (ctx: %p, size: %d)\n", ctx, ctx->workQueueSize);
        // TEMPORARY: Skip fflush to test if this is causing the hang
        // 
        printf("[DEBUG] hasWork: SKIPPED fflush, returning false\n");
    } else {
        printf("[DEBUG] hasWork: returning true\n");
    }
    
    printf("[DEBUG] hasWork: EXIT with result: %d\n", result);
    return result;
}

// Scope Management
static void expandScopeStack(IterativeContext* ctx) {
    int oldCapacity = ctx->scopeStackCapacity;
    ctx->scopeStackCapacity = GROW_CAPACITY(oldCapacity);
    ctx->scopeStack = GROW_ARRAY(ScopeFrame, ctx->scopeStack, oldCapacity, ctx->scopeStackCapacity);
}
...
void pushScope(IterativeContext* ctx, Compiler* compiler, bool isLoopScope) {
    ...
    // Initialize jump arrays using proper memory management
    frame->breakCapacity = 0;  // Will grow as needed
    frame->continueCapacity = 0;  // Will grow as needed
    frame->breakJumps = NULL;
    frame->continueJumps = NULL;
    frame->breakCount = 0;
...
static void expandJumpArray(IterativeContext* ctx) {
    int oldCapacity = ctx->jumpCapacity;
    ctx->jumpCapacity = GROW_CAPACITY(oldCapacity);
    ctx->pendingJumps = GROW_ARRAY(JumpPatch, ctx->pendingJumps, oldCapacity, ctx->jumpCapacity);
}
...
// ============================================================================
// PHASE 1: Basic Expression Processors (for iterative system)
// ============================================================================

bool processLiteral(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    if (!node || !compiler) return false;
    
    // Allocate register for the literal value
    uint16_t reg = allocateRegister(compiler);
    
    // Emit appropriate load instruction based on literal value type
    switch (node->literal.value.type) {
        case VAL_I32:
            emitByte(compiler, OP_LOAD_I32_CONST);
            emitShort(compiler, reg);
            emitBytes(compiler, (node->literal.value.as.i32 >> 8) & 0xFF, node->literal.value.as.i32 & 0xFF);
            break;
        case VAL_I64:
            emitByte(compiler, OP_LOAD_I64_CONST);
            emitShort(compiler, reg);
            // Emit 64-bit value (simplified)
            emitBytes(compiler, (node->literal.value.as.i64 >> 8) & 0xFF, node->literal.value.as.i64 & 0xFF);
            break;
        case VAL_F64:
            emitByte(compiler, OP_LOAD_F64_CONST);
            emitShort(compiler, reg);
            // Store in constants table and emit index
            Value constantValue = {.type = VAL_F64, .as.f64 = node->literal.value.as.f64};
            uint16_t constantIndex = addConstant(compiler->chunk, constantValue);
            emitShort(compiler, constantIndex);
            break;
        case VAL_BOOL:
            emitByte(compiler, node->literal.value.as.boolean ? OP_LOAD_TRUE : OP_LOAD_FALSE);
            emitShort(compiler, reg);
            break;
        case VAL_STRING: {
            // Create string object and store in constants
            Value constantValue = node->literal.value;
            uint16_t constantIndex = addConstant(compiler->chunk, constantValue);
            emitByte(compiler, OP_LOAD_CONST);
            emitShort(compiler, reg);
            emitShort(compiler, constantIndex);
        ...
        default:
            printf("[ITERATIVE] Unsupported literal type: %d\n", node->literal.value.type);
            freeRegister(compiler, reg);
            return false;
    }
    
    printf("[DEBUG] processLiteral: Loaded literal into register %d\n", reg);
    return true;
}
...
bool processIdentifier(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    ...
    int index = findLocal(compiler, node->identifier.name);
    if (index != -1) {
        // Local variable - already in register
        printf("[DEBUG] processIdentifier: Found local '%s' at index %d\n", 
    ...
    printf("[DEBUG] processIdentifier: Processing identifier '%s'\n", node->identifier.name);
    return true;
}
...
bool processAssignment(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    ...
    printf("[DEBUG] processAssignment: Processing assignment to '%s'\n", 
           node->assign.name);
    
    // Handle value expression directly to avoid calling recursive compiler
    int valueReg = -1;
    
    if (node->assign.value->type == NODE_LITERAL) {
        // For literals, allocate a register and emit load constant
        valueReg = allocateRegister(compiler);
        emitConstant(compiler, valueReg, node->assign.value->literal.value);
        printf("[DEBUG] processAssignment: Compiled literal to register %d\n", valueReg);
    } else if (node->assign.value->type == NODE_IDENTIFIER) {
        // For identifiers, find the existing variable register
        int localIndex = findLocal(compiler, node->assign.value->identifier.name);
        if (localIndex >= 0) {
            valueReg = compiler->locals[localIndex].reg;
            printf("[DEBUG] processAssignment: Found variable '%s' in register %d\n", 
                   node->assign.value->identifier.name, valueReg);
        } else {
            printf("[DEBUG] processAssignment: Variable '%s' not found\n", 
                   node->assign.value->identifier.name);
            return false;
        }
    } else if (node->assign.value->type == NODE_BINARY) {
        // Handle binary operations inline to avoid recursive compiler calls
        ASTNode* binOp = node->assign.value;
        int leftReg = -1, rightReg = -1;
        
        // Handle left operand
        if (binOp->binary.left->type == NODE_IDENTIFIER) {
            int localIndex = findLocal(compiler, binOp->binary.left->identifier.name);
            if (localIndex >= 0) {
                leftReg = compiler->locals[localIndex].reg;
            } else {
                printf("[DEBUG] processAssignment: Binary left variable '%s' not found\n", 
                       binOp->binary.left->identifier.name);
                return false;
            }
        } else if (binOp->binary.left->type == NODE_LITERAL) {
            leftReg = allocateRegister(compiler);
            emitConstant(compiler, leftReg, binOp->binary.left->literal.value);
        } else {
            printf("[DEBUG] processAssignment: Unsupported binary left operand type %d\n", 
                   binOp->binary.left->type);
            return false;
        }
        
        // Handle right operand
        if (binOp->binary.right->type == NODE_IDENTIFIER) {
            int localIndex = findLocal(compiler, binOp->binary.right->identifier.name);
            if (localIndex >= 0) {
                rightReg = compiler->locals[localIndex].reg;
            } else {
                printf("[DEBUG] processAssignment: Binary right variable '%s' not found\n", 
                       binOp->binary.right->identifier.name);
                if (binOp->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
                return false;
            }
        } else if (binOp->binary.right->type == NODE_LITERAL) {
            rightReg = allocateRegister(compiler);
            emitConstant(compiler, rightReg, binOp->binary.right->literal.value);
        } else {
            printf("[DEBUG] processAssignment: Unsupported binary right operand type %d\n", 
                   binOp->binary.right->type);
            if (binOp->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
            return false;
        }
        ...
        if (strcmp(binOp->binary.op, ">") == 0) {
            emitByte(compiler, OP_GT_I32_R);
            emitByte(compiler, valueReg);
            emitByte(compiler, leftReg);
            emitByte(compiler, rightReg);
        } else if (strcmp(binOp->binary.op, "<") == 0) {
            emitByte(compiler, OP_LT_I32_R);
            emitByte(compiler, valueReg);
            emitByte(compiler, leftReg);
            emitByte(compiler, rightReg);
            ...
            printf("[DEBUG] processAssignment: Unsupported binary operator '%s'\n", 
                   binOp->binary.op);
            if (binOp->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
            if (binOp->binary.right->type == NODE_LITERAL) freeRegister(compiler, rightReg);
            freeRegister(compiler, valueReg);
            return false;
        }
        
        // Free operand registers if they were allocated for literals
        if (binOp->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
        if (binOp->binary.right->type == NODE_LITERAL) freeRegister(compiler, rightReg);
        
        printf("[DEBUG] processAssignment: Generated binary operation '%s' in register %d\n", 
               binOp->binary.op, valueReg);
    } else {
        printf("[DEBUG] processAssignment: Unsupported value expression type %d\n", 
               node->assign.value->type);
        return false;
    }
    
    if (valueReg < 0) {
        printf("[DEBUG] processAssignment: Failed to get value register\n");
        return false;
    }
    
    // Handle the variable assignment - let addLocal allocate the register
    int localIndex = addLocal(compiler, node->assign.name, true); // true = mutable
    if (localIndex < 0) {
        printf("[DEBUG] processAssignment: Failed to add local variable\n");
        freeRegister(compiler, valueReg);
        return false;
    }
    
    uint16_t varReg = compiler->locals[localIndex].reg;
    ...
    // Free the temporary register used for the value
    freeRegister(compiler, valueReg);
    
    printf("[DEBUG] processAssignment: Variable '%s' assigned to register %d\n", 
           node->assign.name, varReg);
...
bool processBinaryOperation(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    ...
    printf("[DEBUG] processBinaryOperation: Processing binary operation '%s'\n", 
           node->binary.op);
    
    // Compile operands to registers
    int leftReg = -1, rightReg = -1;
    
    // Handle left operand - use existing register assignments
    if (node->binary.left->type == NODE_IDENTIFIER) {
        int localIndex = findLocal(compiler, node->binary.left->identifier.name);
        if (localIndex >= 0) {
            leftReg = compiler->locals[localIndex].reg;
            printf("[DEBUG] processBinaryOperation: Using existing register %d for variable '%s'\n", 
                   leftReg, node->binary.left->identifier.name);
        } else {
            printf("[DEBUG] processBinaryOperation: Left variable '%s' not found\n", 
                   node->binary.left->identifier.name);
            return false;
        }
    } else if (node->binary.left->type == NODE_LITERAL) {
        // For literals in binary operations, allocate a fresh register and load directly
        leftReg = allocateOptimizedRegister(compiler, false, 5);
        Value value = node->binary.left->literal.value;
        
        if (value.type == VAL_I32) {
            emitByte(compiler, OP_LOAD_I32_CONST);
            emitByte(compiler, leftReg);
            emitBytes(compiler, (value.as.i32 >> 8) & 0xFF, value.as.i32 & 0xFF);
            printf("[DEBUG] processBinaryOperation: Loaded literal %d into register %d\n", 
                   value.as.i32, leftReg);
        } else {
            printf("[DEBUG] processBinaryOperation: Unsupported left operand type\n");
            freeRegister(compiler, leftReg);
            return false;
        }
    } else {
        printf("[DEBUG] processBinaryOperation: Unsupported left operand type %d\n", 
               node->binary.left->type);
        return false;
    }
    
    // Handle right operand - use existing register assignments  
    if (node->binary.right->type == NODE_IDENTIFIER) {
        int localIndex = findLocal(compiler, node->binary.right->identifier.name);
        if (localIndex >= 0) {
            rightReg = compiler->locals[localIndex].reg;
            printf("[DEBUG] processBinaryOperation: Using existing register %d for variable '%s'\n", 
                   rightReg, node->binary.right->identifier.name);
        } else {
            printf("[DEBUG] processBinaryOperation: Right variable '%s' not found\n", 
                   node->binary.right->identifier.name);
            if (node->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
            return false;
        }
    } else if (node->binary.right->type == NODE_LITERAL) {
        // For literals in binary operations, allocate a fresh register and load directly
        rightReg = allocateOptimizedRegister(compiler, false, 5);
        Value value = node->binary.right->literal.value;
        ...
        if (value.type == VAL_I32) {
            ...
            emitBytes(compiler, (value.as.i32 >> 8) & 0xFF, value.as.i32 & 0xFF);
            printf("[DEBUG] processBinaryOperation: Loaded literal %d into register %d\n", 
                   value.as.i32, rightReg);
        } else {
            printf("[DEBUG] processBinaryOperation: Unsupported right operand type\n");
            if (node->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
            freeRegister(compiler, rightReg);
            return false;
        }
    } else {
        printf("[DEBUG] processBinaryOperation: Unsupported right operand type %d\n", 
    ...
    // Allocate result register
    uint8_t resultReg = allocateOptimizedRegister(compiler, false, 5);
    ...
    if (strcmp(node->binary.op, "+") == 0) {
        ...
        printf("[BYTECODE] Emitting OP_GT_I32_R: opcode=%02X, dst=%d, src1=%d, src2=%d\n", 
               OP_GT_I32_R, resultReg, leftReg, rightReg);
        emitByte(compiler, OP_GT_I32_R);
        emitByte(compiler, resultReg);
        emitByte(compiler, leftReg);
        emitByte(compiler, rightReg);
        ...
        printf("[DEBUG] processBinaryOperation: Unsupported operator '%s'\n", node->binary.op);
        if (node->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
        if (node->binary.right->type == NODE_LITERAL) freeRegister(compiler, rightReg);
        freeRegister(compiler, resultReg);
        return false;
    }
    
    printf("[DEBUG] processBinaryOperation: Generated %s operation in register %d\n", 
           node->binary.op, resultReg);
...
bool processIfStatement(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    if (!node || !compiler || node->type != NODE_IF) return false;
    
    printf("[DEBUG] processIfStatement: Processing if statement\n");
    
    // Compile condition - for now, simplified approach for identifiers only
    printf("[DEBUG] processIfStatement: Compiling condition\n");
    int conditionReg = -1;
    
    if (node->ifStmt.condition->type == NODE_IDENTIFIER) {
        int localIndex = findLocal(compiler, node->ifStmt.condition->identifier.name);
        if (localIndex >= 0) {
            conditionReg = compiler->locals[localIndex].reg;
            printf("[DEBUG] processIfStatement: Using variable '%s' in register %d as condition\n", 
                   node->ifStmt.condition->identifier.name, conditionReg);
        } else {
            printf("[DEBUG] processIfStatement: Condition variable '%s' not found\n", 
                   node->ifStmt.condition->identifier.name);
            return false;
        }
    } else if (node->ifStmt.condition->type == NODE_BINARY) {
        // Handle comparison operations like x > 5
        printf("[DEBUG] processIfStatement: Compiling binary comparison condition\n");
        ASTNode* cond = node->ifStmt.condition;
        
        // Compile left operand
        int leftReg = -1;
        if (cond->binary.left->type == NODE_IDENTIFIER) {
            int localIndex = findLocal(compiler, cond->binary.left->identifier.name);
            if (localIndex >= 0) {
                leftReg = compiler->locals[localIndex].reg;
            } else {
                printf("[DEBUG] processIfStatement: Left operand variable '%s' not found\n", 
                       cond->binary.left->identifier.name);
                return false;
            }
        } else if (cond->binary.left->type == NODE_LITERAL) {
            leftReg = allocateOptimizedRegister(compiler, false, 5);
            emitConstant(compiler, leftReg, cond->binary.left->literal.value);
            printf("[DEBUG] processIfStatement: Loaded left literal into register %d\n", leftReg);
        } else {
            printf("[DEBUG] processIfStatement: Unsupported left operand type %d\n", cond->binary.left->type);
            return false;
        }
        
        // Compile right operand  
        int rightReg = -1;
        if (cond->binary.right->type == NODE_IDENTIFIER) {
            int localIndex = findLocal(compiler, cond->binary.right->identifier.name);
            if (localIndex >= 0) {
                rightReg = compiler->locals[localIndex].reg;
            } else {
                printf("[DEBUG] processIfStatement: Right operand variable '%s' not found\n", 
                       cond->binary.right->identifier.name);
                if (cond->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
                return false;
            }
        } else if (cond->binary.right->type == NODE_LITERAL) {
            rightReg = allocateOptimizedRegister(compiler, false, 5);
            emitConstant(compiler, rightReg, cond->binary.right->literal.value);
            printf("[DEBUG] processIfStatement: Loaded right literal into register %d\n", rightReg);
        } else {
            printf("[DEBUG] processIfStatement: Unsupported right operand type %d\n", cond->binary.right->type);
            if (cond->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
            return false;
        }
        
        // Allocate result register for comparison
        conditionReg = allocateOptimizedRegister(compiler, false, 5);
        
        // Emit comparison operation
        if (strcmp(cond->binary.op, ">") == 0) {
            emitByte(compiler, OP_GT_I32_R);
            emitByte(compiler, conditionReg);
            emitByte(compiler, leftReg);
            emitByte(compiler, rightReg);
        } else if (strcmp(cond->binary.op, "<") == 0) {
            emitByte(compiler, OP_LT_I32_R);
            emitByte(compiler, conditionReg);
            emitByte(compiler, leftReg);
            emitByte(compiler, rightReg);
            ...
            printf("[DEBUG] processIfStatement: Unsupported comparison operator '%s'\n", cond->binary.op);
            if (cond->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
            if (cond->binary.right->type == NODE_LITERAL) freeRegister(compiler, rightReg);
            freeRegister(compiler, conditionReg);
            return false;
        }
        
        printf("[DEBUG] processIfStatement: Generated %s comparison in register %d\n", 
               cond->binary.op, conditionReg);
        
        // Free operand registers if they were allocated for literals
        if (cond->binary.left->type == NODE_LITERAL) freeRegister(compiler, leftReg);
        if (cond->binary.right->type == NODE_LITERAL) freeRegister(compiler, rightReg);
    } else {
        printf("[DEBUG] processIfStatement: Unsupported condition type %d - falling back to expression compiler\n", 
               node->ifStmt.condition->type);
        conditionReg = compileExpressionToRegister(node->ifStmt.condition, compiler);
        if (conditionReg < 0) {
            printf("[DEBUG] processIfStatement: Failed to compile condition\n");
            return false;
        }
    }
    ...
    // For the iterative system, we need to handle jump patching differently
    // We can't patch immediately because the then branch hasn't been compiled yet
    // Let's use a simpler approach: compile the then branch inline if it's simple
    
    if (node->ifStmt.thenBranch) {
        printf("[DEBUG] processIfStatement: Compiling then branch inline\n");
        
        // Compile then branch immediately (not queued)
        bool thenSuccess = true;
        if (node->ifStmt.thenBranch->type == NODE_BLOCK) {
            // Handle block inline by processing its statements
            for (int i = 0; i < node->ifStmt.thenBranch->block.count; i++) {
                ASTNode* stmt = node->ifStmt.thenBranch->block.statements[i];
                if (stmt->type == NODE_PRINT) {
                    // Handle print statements inline
        ...
        if (!thenSuccess) {
            printf("[ERROR] processIfStatement: Failed to compile then branch\n");
            return false;
        }
    }
    
    // Now we can patch the jump correctly since the then branch is compiled
    printf("[DEBUG] processIfStatement: Patching then jump from %d to %d\n", 
           thenJump, compiler->chunk->count);
    patchJump(compiler, thenJump);
    
    // If there's an else branch, emit jump over it
    int elseJump = -1;
    if (node->ifStmt.elseBranch) {
        printf("[DEBUG] processIfStatement: Has else branch, emitting else jump\n");
        emitByte(compiler, OP_JUMP);
        elseJump = emitJump(compiler);
        
        // Patch the else jump immediately since we don't have else branch content yet
        patchJump(compiler, elseJump);
    }
    
    printf("[DEBUG] processIfStatement: If statement processing complete\n");
    return true;
}

bool processWhileStatement(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    if (!node || !compiler || node->type != NODE_WHILE) return false;
    
    printf("[DEBUG] processWhileStatement: Processing while statement\n");
    
    // Mark loop start
    int loopStart = compiler->chunk->count;
    
    // Compile condition
    printf("[DEBUG] processWhileStatement: Compiling condition\n");
    int conditionReg = compileExpressionToRegister(node->whileStmt.condition, compiler);
    if (conditionReg < 0) {
        printf("[DEBUG] processWhileStatement: Failed to compile condition\n");
        return false;
    }
    
    // Emit conditional jump (jump if false - exit loop)
    emitByte(compiler, OP_JUMP_IF_NOT_R);
    emitByte(compiler, conditionReg);
    int exitJump = emitJump(compiler);
    
    // Compile body
    printf("[DEBUG] processWhileStatement: Compiling body\n");
    if (node->whileStmt.body) {
        pushWork(ctx, node->whileStmt.body, WORK_NODE_NORMAL, 10);
    }
    ...
    printf("[DEBUG] processWhileStatement: While statement processing complete\n");
    return true;
}
...
bool processUnaryOperation(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    ...
    printf("[DEBUG] processUnaryOperation: Processing unary operation '%s'\n", 
           node->unary.op);
    
    // Queue operand for processing
    pushWork(ctx, node->unary.operand, WORK_NODE_NORMAL, 1);
...
bool processCast(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    ...
    printf("[DEBUG] processCast: Processing cast operation\n");
    
    // Queue expression for processing
    pushWork(ctx, node->cast.expression, WORK_NODE_NORMAL, 1);
...
bool processExpression(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    // Handle different expression types in the iterative system
    switch (node->type) {
        case NODE_LITERAL:
            return processLiteral(node, compiler, ctx);
        case NODE_IDENTIFIER:
            return processIdentifier(node, compiler, ctx);
        case NODE_ASSIGN:
            return processAssignment(node, compiler, ctx);
        case NODE_BINARY:
            return processBinaryOperation(node, compiler, ctx);
        case NODE_UNARY:
            return processUnaryOperation(node, compiler, ctx);
        case NODE_CAST:
            return processCast(node, compiler, ctx);
        default:
            printf("[ITERATIVE] Unsupported expression type: %d\n", node->type);
            return false;
    }
}

bool processPrint(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    printf("[DEBUG] processPrint: Processing print statement with %d arguments\n", node->print.count);
    ...
    if (node->print.count == 0) {
        ...
        // Single argument print - compile the argument using the old system for now
        ASTNode* arg = node->print.values[0];
        
        // Use the iterative system to compile the argument expression
        int valueReg = -1;
        
        // For simple identifiers, handle directly
        if (arg->type == NODE_IDENTIFIER) {
            int localIndex = findLocal(compiler, arg->identifier.name);
            if (localIndex >= 0) {
                valueReg = compiler->locals[localIndex].reg;
                printf("[DEBUG] processPrint: Found variable '%s' in register %d\n", arg->identifier.name, valueReg);
            } else {
                printf("[DEBUG] processPrint: Variable '%s' not found\n", arg->identifier.name);
                return false;
            }
        } else if (arg->type == NODE_LITERAL) {
            // For literals, use the same approach as binary expressions - delegate to working system
            printf("[DEBUG] processPrint: Attempting to compile literal using compileExpressionToRegister\n");
            valueReg = compileExpressionToRegister(arg, compiler);
            if (valueReg < 0) {
                printf("[DEBUG] processPrint: Failed to compile literal expression\n");
                return false;
            }
            printf("[DEBUG] processPrint: Literal compiled to register %d\n", valueReg);
        } else if (arg->type == NODE_BINARY) {
            // For binary expressions, compile them by calling the old working system
            printf("[DEBUG] processPrint: Attempting to compile binary expression using compileExpressionToRegister\n");
            valueReg = compileExpressionToRegister(arg, compiler);
            if (valueReg < 0) {
                printf("[DEBUG] processPrint: Failed to compile binary expression\n");
                return false;
            }
            printf("[DEBUG] processPrint: Binary expression compiled to register %d\n", valueReg);
        } else {
            printf("[DEBUG] processPrint: Unsupported expression type %d in print\n", arg->type);
            return false;
        }
        
        if (valueReg < 0) {
            printf("[DEBUG] processPrint: Failed to compile argument expression\n");
            return false;
        }
        ...
        printf("[DEBUG] processPrint: Successfully processed single-argument print\n");
        return true;
    } else {
        // Multi-argument print - use proper arena allocation
        int oldCapacity = 0;
        uint8_t* argRegs = GROW_ARRAY(uint8_t, NULL, oldCapacity, node->print.count);
        
        // Allocate registers for all arguments
        ...
        // Compile each argument using the iterative system
        for (int i = 0; i < node->print.count; i++) {
            ASTNode* arg = node->print.values[i];
            int valueReg = -1;
            ...
            if (arg->type == NODE_IDENTIFIER) {
                ...
                if (localIndex >= 0) {
                    ...
                    printf("[DEBUG] processPrint: Variable '%s' not found in multi-print\n", arg->identifier.name);
                    for (int j = 0; j < node->print.count; j++) {
                        freeRegister(compiler, argRegs[j]);
                    }
                    FREE_ARRAY(uint8_t, argRegs, node->print.count);
                    return false;
                }
            } else if (arg->type == NODE_LITERAL) {
                valueReg = allocateOptimizedRegister(compiler, false, 5);
                Value value = arg->literal.value;
                ...
                if (value.type == VAL_I32) {
                    ...
                    // Store string in constants table and emit load
                    int constantIndex = addConstant(compiler->chunk, value);
                    emitByte(compiler, OP_LOAD_CONST);
                    emitByte(compiler, valueReg);
                    emitShort(compiler, constantIndex);
                } else {
                    printf("[DEBUG] processPrint: Unsupported literal type %d in multi-print\n", value.type);
                    freeRegister(compiler, valueReg);
                    for (int j = 0; j < node->print.count; j++) {
                        freeRegister(compiler, argRegs[j]);
                    }
                ...
                printf("[DEBUG] processPrint: Unsupported expression type %d in multi-print\n", arg->type);
                for (int j = 0; j < node->print.count; j++) {
                    freeRegister(compiler, argRegs[j]);
                }
                FREE_ARRAY(uint8_t, argRegs, node->print.count);
        ...
        // Free the array using arena allocation
        FREE_ARRAY(uint8_t, argRegs, node->print.count);
        
        printf("[DEBUG] processPrint: Successfully processed multi-argument print\n");
        return true;
    }
}
...
bool compileIterative(ASTNode* ast, Compiler* compiler) {
    printf("[DEBUG] compileIterative: Starting\n");
    
    IterativeContext ctx;
    initIterativeContext(&ctx);
    
    // Initialize with root node
    printf("[DEBUG] compileIterative: About to push root node type %d\n", ast->type);
    
    pushWork(&ctx, ast, WORK_NODE_NORMAL, 1);
    printf("[DEBUG] compileIterative: Root node pushed, queue size: %d\n", ctx.workQueueSize);
    
    
    int iterations = 0;
    printf("[DEBUG] compileIterative: About to enter main loop\n");
    
    while (hasWork(&ctx)) {
        iterations++;
        printf("[DEBUG] compileIterative: Iteration %d, queue size: %d\n", iterations, ctx.workQueueSize);
        
        if (iterations > 100) {
            printf("[ERROR] compileIterative: Too many iterations, breaking to prevent infinite loop\n");
            break;
        }
        ...
        if (!item) {
            printf("[DEBUG] compileIterative: popWork returned NULL\n");
            break;
        }
        
        printf("[DEBUG] compileIterative: Processing node type %d\n", item->node ? item->node->type : -1);
        
        ctx.nodesProcessed++;
        
        // Handle special node types
        if (item->type == WORK_NODE_SCOPE_END) {
            printf("[DEBUG] compileIterative: Handling SCOPE_END\n");
            popScope(&ctx, compiler);
            continue;
        }
        
        if (item->type == WORK_NODE_LOOP_PATCH) {
            printf("[DEBUG] compileIterative: Handling LOOP_PATCH\n");
            continue;
        }
        
        // Process regular nodes
        printf("[DEBUG] compileIterative: About to call processNode\n");
        if (!processNode(item->node, compiler, &ctx)) {
            printf("[DEBUG] compileIterative: processNode failed\n");
            cleanupIterativeContext(&ctx);
            return false;
        }
        printf("[DEBUG] compileIterative: processNode succeeded\n");
    }
    
    printf("[DEBUG] compileIterative: WHILE LOOP EXITED - iterations: %d\n", iterations);
    printf("[DEBUG] compileIterative: ctx.nodesProcessed: %d\n", ctx.nodesProcessed);
    printf("[DEBUG] compileIterative: ctx.workQueueSize: %d\n", ctx.workQueueSize);
    printf("[DEBUG] compileIterative: About to return true\n");
    return true;
}

bool processNode(ASTNode* node, Compiler* compiler, IterativeContext* ctx) {
    if (!node) {
        printf("[DEBUG] processNode: node is NULL\n");
        return true;
    }
    
    printf("[DEBUG] processNode: Processing node type %d\n", node->type);
    ...
    switch (node->type) {
        case NODE_PROGRAM:
            printf("[DEBUG] processNode: Handling NODE_PROGRAM\n");
            return processProgram(node, compiler, ctx);
        case NODE_BLOCK:
            printf("[DEBUG] processNode: Handling NODE_BLOCK\n");
            return processBlock(node, compiler, ctx);
        case NODE_FOR_RANGE:
            printf("[DEBUG] processNode: Handling NODE_FOR_RANGE\n");
            return processForRange(node, compiler, ctx);
        case NODE_IF:
            printf("[DEBUG] processNode: Handling NODE_IF iteratively\n");
            return processIfStatement(node, compiler, ctx);
        case NODE_WHILE:
            printf("[DEBUG] processNode: Handling NODE_WHILE iteratively\n");
            return processWhileStatement(node, compiler, ctx);
        default:
            printf("[DEBUG] processNode: Handling as leaf node, type %d\n", node->type);
...
bool compileMultiPass(ASTNode* ast, Compiler* compiler, bool isModule) {
    ...
    printf("[DEBUG] compileMultiPass: About to get mpCompiler\n");
    
    MultiPassCompiler* mpCompiler = g_multiPassCompiler;
    printf("[DEBUG] compileMultiPass: Got mpCompiler: %p\n", mpCompiler);
    

    // Initialize scope analyzer (TEMPORARILY DISABLED for debugging)
    printf("[DEBUG] compileMultiPass: About to skip scope analysis\n");
    
    // initCompilerScopeAnalysis(compiler);
    printf("[DEBUG] compileMultiPass: Scope analysis skipped\n");
    

    // PASS 1: Type analysis (placeholder for now)
    printf("[DEBUG] compileMultiPass: About to set typeAnalysisComplete\n");
    
    mpCompiler->typeAnalysisComplete = true;
    printf("[DEBUG] compileMultiPass: typeAnalysisComplete set\n");
    

    // PASS 2: Scope analysis - now using comprehensive system
    // The scope analysis happens incrementally during compilation
    printf("[DEBUG] compileMultiPass: About to set scopeAnalysisComplete\n");
    
    mpCompiler->scopeAnalysisComplete = true;
    printf("[DEBUG] compileMultiPass: scopeAnalysisComplete set\n");
    

    // PASS 3: Main compilation with iterative system
    printf("[DEBUG] compileMultiPass: About to call compileIterative (step 1)\n");
    
    printf("[DEBUG] compileMultiPass: About to call compileIterative (step 2)\n");
    
    printf("[DEBUG] compileMultiPass: About to call compileIterative\n");
    
    bool success = compileIterative(ast, compiler);
    printf("[DEBUG] compileIterative returned: %d\n", success);
...
bool compileExpression(ASTNode* node, Compiler* compiler) {
    printf("[PHASE1] compileExpression called - using iterative system\n");
    
    // Use iterative compilation for expressions
    IterativeContext ctx;
    initIterativeContext(&ctx);
    
    // Process the expression node iteratively
    bool success = processNode(node, compiler, &ctx);
    
    cleanupIterativeContext(&ctx);
    printf("[PHASE1] compileExpression completed, success: %d\n", success);
    return success;
}
