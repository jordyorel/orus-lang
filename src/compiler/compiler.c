...
static void patch_jump_list(JumpList* list, Compiler* compiler, int target) {
    for (int i = 0; i < list->count; i++) {
        if (list->jumps[i].target == -1) {
            ...
            // Patch the jump offset (handle overflow gracefully)
            if (offset >= 0 && offset <= 255) {
                compiler->chunk->code[address] = (uint8_t)offset;
            } else {
                // Use long jump format or emit error
                compiler->chunk->code[address] = 255; // Max offset, will cause runtime jump
            }
