...
static void promoteLoopInvariantVariables(Compiler* compiler, int loopStart,
                                          int loopEnd);

// Forward declarations for enhanced variable lifetime analysis
static void markVariableAsEscaping(RegisterAllocator* allocator,
                                   int localIndex);
static bool isVariableLoopInvariant(Compiler* compiler, int localIndex);
static bool isVariableModifiedInRange(Compiler* compiler, int localIndex,
                                      int startInstr, int endInstr);
static int findLocalByRegister(Compiler* compiler, int regIndex);
static void sortSpillCandidatesByPriority(int* candidates, int count,
...
static bool isRegisterLoopInvariant(Compiler* compiler, uint8_t reg,
...
static bool isUsedInMultiplication(Compiler* compiler, int localIndex,
                                   int loopStart, int loopEnd);
static bool canSafelyHoistInstruction(InvariantNode* node, LoopContext* loopCtx,
                                      Compiler* compiler);
static bool hasInstructionSideEffects(uint8_t instruction);
static bool isRegisterUsedAfterLoop(Compiler* compiler, uint8_t reg,
                                    LoopContext* loopCtx);
static bool insertInstructionSpace(Compiler* compiler, int offset, int size);
static void updateJumpTargetsAfterInsertion(Compiler* compiler, int insertPos,
                                            int insertSize);
...
static void hoistInvariantCodeInstruction(Compiler* compiler,
...
__attribute__((unused)) static bool isRegisterTyped(Compiler* compiler,
                                                    uint8_t reg) {
    ...
    for (int i = 0; i < compiler->pendingJumps.offsets.count; i++) {
        if (compiler->pendingJumps.offsets.data[i] == offset) {
            ...
            compiler->pendingJumps.offsets.data[i] =
                compiler->pendingJumps.offsets
                    .data[compiler->pendingJumps.offsets.count - 1];
...
static void patchJump(Compiler* compiler, int offset) {
    int jump =
        compiler->chunk->count - offset - 1;  // -1 for 1-byte placeholder
    ...
    if (jump > 255) {
        ...
        jump =
            compiler->chunk->count - offset - 2;  // -2 for 2-byte placeholder
...
static void emitLoop(Compiler* compiler, int loopStart) {
    int offset =
        compiler->chunk->count - loopStart + 2;  // +2 for short, +3 for long
...
static void exitScope(Compiler* compiler) {
    ...
    while (compiler->localCount > targetCount) {
        ...
        // Handle variable shadowing properly - restore previous variable
        // instead of removing
        const char* varName = compiler->locals[localIndex].name;

        // Look for a previous variable with the same name at a lower scope
        // depth
    ...
    // Look for opportunities to reuse registers from variables that just went
    // out of scope
    ...
    for (int i = 0; i < allocator->count; i++) {
        ...
        if (range->end == currentInstr && !range->spilled) {
            // This variable just ended - its register is now available for
            // reuse The register should already be added to freeRegs by
            // endVariableLifetime
        }
    }
}
...
static ValueType inferBinaryOpTypeWithCompiler(ASTNode* left, ASTNode* right,
    ...
    // u64 and i64 - both are 64-bit, but we need exact type matches for
    // arithmetic If one is u64 and other is i64, this is a type error, but
    // we'll return i64 for now
...
int compileExpressionToRegister(ASTNode* node, Compiler* compiler) {
    ...
    switch (node->type) {
        ...
        case NODE_UNARY: {
            int operandReg =
                compileExpressionToRegister(node->unary.operand, compiler);
        ...
        case NODE_BINARY: {
            ...
            int leftReg =
                compileExpressionToRegister(node->binary.left, compiler);
            if (leftReg < 0) return -1;
            int rightReg =
                compileExpressionToRegister(node->binary.right, compiler);
            ...
            uint8_t resultReg =
                leftTemp ? (uint8_t)leftReg : allocateRegister(compiler);

            // Determine operation type and handle type conversion
            ValueType leftType =
                getNodeValueTypeWithCompiler(node->binary.left, compiler);
            ValueType rightType =
                getNodeValueTypeWithCompiler(node->binary.right, compiler);
            ValueType opType = inferBinaryOpTypeWithCompiler(
            ...
            // Phase 3.2: Check if we can emit typed instructions based on type
            // inference
            ValueType typedOpType;
            bool canUseTyped = canEmitTypedInstruction(
                compiler, node->binary.left, node->binary.right, &typedOpType);
            ...
            if (strcmp(op, "+") == 0) {
                ...
                // Phase 2.3: Instruction fusion for immediate arithmetic
                    ...
                    emitByte(compiler, (uint8_t)leftReg);
                    int32_t immediate =
                        AS_I32(node->binary.right->literal.value);
                ...
                // Phase 3.2: Use typed instructions when types are known at
                // compile time
                else if (canUseTyped) {
                    emitTypedBinaryOp(compiler, op, typedOpType, resultReg,
                                      (uint8_t)leftReg, (uint8_t)rightReg);
                } else if (opType == VAL_I64) {
                ...
                // Implement short-circuit AND using existing binary operation
                // for now
                // TODO: Implement proper short-circuit evaluation
                emitByte(compiler, OP_AND_BOOL_R);
            } else if (strcmp(op, "or") == 0) {
                // Implement short-circuit OR using existing binary operation
                // for now
                // TODO: Implement proper short-circuit evaluation
        ...
        case NODE_VAR_DECL: {
            // Determine variable type from type annotation FIRST (before
            // compiling expression)
            ValueType declaredType;
            bool hasTypeAnnotation =
                node->varDecl.typeAnnotation &&
                node->varDecl.typeAnnotation->typeAnnotation.name;
            ...
            if (hasTypeAnnotation) {
                ...
                // CRITICAL FIX: Coerce literal types BEFORE compiling the
                // expression This ensures the literal is compiled with the
                // correct type
                if (node->varDecl.initializer &&
                    ...
                    // Check if this is a plain integer literal that can be
                    // coerced
                    if ((literalType == VAL_I32 || literalType == VAL_I64) &&
                        (declaredType == VAL_U32 || declaredType == VAL_U64 ||
                         declaredType == VAL_I32 || declaredType == VAL_I64)) {
                        // Get the integer value and coerce it to the declared
                        // type
                        int64_t intValue;
                        ...
                        if (declaredType == VAL_U32) {
                            if (intValue >= 0 && intValue <= UINT32_MAX) {
                                literal->literal.value =
                                    U32_VAL((uint32_t)intValue);
                            }
                        } else if (declaredType == VAL_U64) {
                            if (intValue >= 0) {
                                literal->literal.value =
                                    U64_VAL((uint64_t)intValue);
                            }
                ...
                declaredType = getNodeValueTypeWithCompiler(
                    node->varDecl.initializer, compiler);
            }

            // NOW compile the expression with the correctly typed literal
            int initReg = compileExpressionToRegister(node->varDecl.initializer,
                                                      compiler);
            ...
            symbol_table_set(&compiler->symbols, node->varDecl.name,
                             localIndex);

            // Handle type conversion if needed
            // For complex expressions, we need to determine the actual result
            // type
            ValueType initType;
            if (node->varDecl.initializer->type == NODE_LITERAL) {
                ...
                initType = inferBinaryOpTypeWithCompiler(
                    ...
                    node->varDecl.initializer->binary.right, compiler);
            } else {
            ...
            compilerDeclareVariable(compiler, node->varDecl.name, declaredType,
                                    (uint8_t)initReg);
            ...
            if (node->varDecl.initializer->type == NODE_LITERAL) {
                ...
                compiler->locals[localIndex].knownType =
                    declaredType;  // Use final type after any conversions
            } else {
        ...
        case NODE_ASSIGN: {
            ...
            uint8_t valueReg =
                compileExpressionToRegister(node->assign.value, compiler);
            ...
            if (localIndex < 0) {
                ...
                compiler->locals[localIndex].type =
                    getNodeValueTypeWithCompiler(node->assign.value, compiler);
                symbol_table_set(&compiler->symbols, node->assign.name,
                                 localIndex);
            ...
            // Phase 3.1: Update safe type tracking for existing variable
            // reassignments
            if (node->assign.value->type == NODE_LITERAL) {
        ...
        case NODE_PRINT: {
            ...
            for (int i = 0; i < node->print.count; i++) {
                int src = compileExpressionToRegister(node->print.values[i],
                                                      compiler);
                if (src < 0) return -1;
                uint8_t dest = regs[i];
                if (src != dest) {
        ...
        case NODE_BLOCK: {
            ...
            for (int i = 0; i < node->block.count; i++) {
                int result = compileExpressionToRegister(
                    node->block.statements[i], compiler);
                ...
                if (i == node->block.count - 1) {
                    ...
                    // For non-last statements, we might want to free temporary
                    // registers but for now, just keep the behavior simple
                }
            }
            return lastResult;
        }
        case NODE_IF: {
            int cond =
                compileExpressionToRegister(node->ifStmt.condition, compiler);
            if (cond < 0) return -1;
            int elseJump = emitConditionalJump(compiler, (uint8_t)cond);
            freeRegister(compiler, (uint8_t)cond);

            enterScope(compiler);
            if (compileExpressionToRegister(node->ifStmt.thenBranch, compiler) <
                0) {
                exitScope(compiler);
                return -1;
            }
            ...
            if (node->ifStmt.elseBranch) {
                enterScope(compiler);
                if (compileExpressionToRegister(node->ifStmt.elseBranch,
                                                compiler) < 0) {
                    exitScope(compiler);
                    return -1;
                }
        ...
        case NODE_WHILE: {
            ...
            int condReg = compileExpressionToRegister(node->whileStmt.condition,
                                                      compiler);
            if (condReg < 0) {
                exitLoop(compiler);
                return -1;
            ...
            if (compileExpressionToRegister(node->whileStmt.body, compiler) <
                0) {
                exitScope(compiler);
                exitLoop(compiler);
                return -1;
            ...
            LoopContext* currentLoopCtx =
                &compiler->loopStack[compiler->loopDepth - 1];
        ...
        case NODE_FOR_RANGE: {
            int startReg =
                compileExpressionToRegister(node->forRange.start, compiler);
            if (startReg < 0) return -1;
            int endReg =
                compileExpressionToRegister(node->forRange.end, compiler);
            if (endReg < 0) {
            ...
            if (node->forRange.step) {
                stepReg =
                    compileExpressionToRegister(node->forRange.step, compiler);
                if (stepReg < 0) {
                    freeRegister(compiler, (uint8_t)startReg);
            ...
            // Use enhanced register allocation with lifetime tracking for loop
            // variable Ensure loop variable gets a low register number to avoid
            // conflicts
            ...
            compiler->locals[localIndex].type =
                VAL_I32;  // for range loops use i32
            // Initialize safe type tracking fields
            compiler->locals[localIndex].hasKnownType =
                false;  // Loop variables change, so not safe to track
            compiler->locals[localIndex].knownType = VAL_NIL;
            symbol_table_set(&compiler->symbols, node->forRange.varName,
                             localIndex);

            // Connect local to live range in register allocator
            int rangeIndex =
                compiler->regAlloc.count - 1;  // Most recently added range
            ...
            // Use optimized register reuse for condition register (ensure it's
            // different from guard)
            uint8_t condReg = allocateRegister(compiler);
            emitByte(compiler,
                     node->forRange.inclusive ? OP_LE_I32_R : OP_LT_I32_R);
            ...
            if (compileExpressionToRegister(node->forRange.body, compiler) <
                0) {
                exitLoop(compiler);
                exitScope(compiler);
                return -1;
            ...
            // analyzeVariableEscapes(compiler, 1); // This is a single loop
            // level optimizeRegisterPressure(compiler);
        ...
        case NODE_FOR_ITER: {
            int iterSrc =
                compileExpressionToRegister(node->forIter.iterable, compiler);
            ...
            uint8_t iterator =
                reuseOrAllocateRegister(compiler, "_iterator", VAL_ARRAY);
            ...
            // Use enhanced register allocation with lifetime tracking for loop
            // variable
            uint8_t loopVar = allocateRegisterWithLifetime(
                compiler, node->forIter.varName, VAL_I64, true);
            int localIndex = compiler->localCount++;
            ...
            compiler->locals[localIndex].isMutable = true;
            compiler->locals[localIndex].type =
                VAL_I64;  // iterator values are i64
            // Initialize safe type tracking fields
            compiler->locals[localIndex].hasKnownType =
                false;  // Loop variables change, so not safe to track
            compiler->locals[localIndex].knownType = VAL_NIL;
            symbol_table_set(&compiler->symbols, node->forIter.varName,
                             localIndex);
            ...
            uint8_t hasReg =
                reuseOrAllocateRegister(compiler, "_iter_has_next", VAL_BOOL);
        ...
        case NODE_BREAK: {
            LoopContext* currentLoop =
                getLoopByLabel(compiler, node->breakStmt.label);
            if (!currentLoop) {
            ...
            jumptable_add(&currentLoop->breakJumps,
                          emitJump(compiler, OP_JUMP_SHORT));
            return 0;
        }
        case NODE_CONTINUE: {
            LoopContext* currentLoop =
                getLoopByLabel(compiler, node->continueStmt.label);
            if (!currentLoop) {
            ...
            jumptable_add(&currentLoop->continueJumps,
                          emitJump(compiler, OP_JUMP_SHORT));
            return 0;
        }
        case NODE_TERNARY: {
            int cond =
                compileExpressionToRegister(node->ternary.condition, compiler);
            if (cond < 0) return -1;
            uint8_t resultReg = allocateRegister(compiler);
            int falseJump = emitConditionalJump(compiler, (uint8_t)cond);

            int trueReg =
                compileExpressionToRegister(node->ternary.trueExpr, compiler);
            ...
            int falseReg =
                compileExpressionToRegister(node->ternary.falseExpr, compiler);
        ...
        case NODE_FUNCTION: {
            ...
            initCompiler(&functionCompiler, functionChunk, compiler->fileName,
                         compiler->source);
            ...
            for (int i = 0; i < node->function.paramCount; i++) {
                ...
                // Add to symbol table for lookup - CRITICAL: Use current
                // localCount before incrementing
                symbol_table_set(&functionCompiler.symbols, paramName,
                                 currentLocalIndex);
            ...
            int resultReg = compileExpressionToRegister(node->function.body,
                                                        &functionCompiler);
            if (resultReg < 0) {
            ...
            compiler->locals[localIndex].isMutable = false;
            compiler->locals[localIndex].type =
                VAL_I32;  // function index is stored as i32
            symbol_table_set(&compiler->symbols, node->function.name,
                             localIndex);
        ...
        case NODE_CALL: {
            // Compile function call: func_reg, first_arg_reg, arg_count,
            // result_reg
            int funcReg =
                compileExpressionToRegister(node->call.callee, compiler);
            ...
            if (node->call.argCount > 0) {
                ...
                // Compile each argument and move to consecutive registers if
                // needed
                for (int i = 0; i < node->call.argCount; i++) {
                    int argReg = compileExpressionToRegister(node->call.args[i],
                                                             compiler);
                    if (argReg < 0) {
                    ...
                    // Move argument to consecutive register if not already
                    // there
        ...
        case NODE_RETURN: {
            if (node->returnStmt.value) {
                // Return with value
                int valueReg = compileExpressionToRegister(
                    node->returnStmt.value, compiler);
                if (valueReg < 0) return -1;
...
void initCompiler(Compiler* compiler, Chunk* chunk, const char* fileName,
    ...
    // Phase 3.1: Initialize register type tracking (disabled - causes variable
    // resolution issues) initRegisterTypes(compiler);
...
static int addLiveRange(RegisterAllocator* allocator, const char* name,
                        uint8_t reg, ValueType type, int start,
                        bool isLoopVar) {
    if (allocator->count >= allocator->capacity) {
        allocator->capacity *= 2;
        allocator->ranges =
            realloc(allocator->ranges, sizeof(LiveRange) * allocator->capacity);
    }
...
uint8_t allocateRegisterWithLifetime(Compiler* compiler, const char* name,
                                     ValueType type, bool isLoopVar) {
    RegisterAllocator* allocator = &compiler->regAlloc;
    int currentInstr = compiler->chunk->count;
...
uint8_t reuseOrAllocateRegister(Compiler* compiler, const char* name,
                                ValueType type) {
    RegisterAllocator* allocator = &compiler->regAlloc;
    int currentInstr = compiler->chunk->count;
...
void optimizeLoopVariableLifetimes(Compiler* compiler, int loopStart,
                                   int loopEnd) {
    RegisterAllocator* allocator = &compiler->regAlloc;
    ...
    for (int i = 0; i < allocator->count; i++) {
        ...
        if (range->isLoopVar && range->start >= loopStart &&
            range->start <= loopEnd) {
            // This is a loop variable - extend its lifetime to cover the entire
            // loop
            if (range->end == -1 || range->end < loopEnd) {
                range->end = loopEnd;
            }

            // Mark this register as preferentially allocated for loop
            // performance (Implementation could include hints for register
            // allocator)
        }
    }
...
__attribute__((unused)) static void analyzeVariableEscapes(Compiler* compiler,
                                                           int loopDepth) {
    RegisterAllocator* allocator = &compiler->regAlloc;
    ...
    for (int i = 0; i < compiler->localCount; i++) {
        if (compiler->locals[i].isActive &&
            compiler->locals[i].depth >= compiler->scopeDepth - loopDepth) {
            ...
            if (rangeIndex >= 0 && rangeIndex < allocator->count) {
                ...
                if (range->end == -1 || range->end > compiler->chunk->count) {
                    // Variable lifetime extends beyond current point - it might
                    // escape
                    markVariableAsEscaping(allocator, i);

                    // Check if variable is used in nested loops - affects
                    // optimization decisions
                    if (compiler->loopDepth > 1) {
                        // Variable used in nested loop context - requires
                        // special handling
                        range->nestedLoopUsage = true;
                    }

                    // Track cross-loop dependencies
                    if (range->firstUse < compiler->loopStart &&
                        range->lastUse > compiler->loopStart) {
                        range->crossesLoopBoundary = true;
                    }
                } else {
                    // Variable lifetime is contained within current scope
                    if (range->lastUse - range->firstUse < 5) {
                        // Short-lived variable - candidate for aggressive
                        // optimization
...
__attribute__((unused)) static void optimizeRegisterPressure(
    Compiler* compiler) {
    RegisterAllocator* allocator = &compiler->regAlloc;
    int currentInstr = compiler->chunk->count;
    ...
    // Find registers that haven't been used recently and could be candidates
    // for spilling
    for (int i = 0; i < REGISTER_COUNT; i++) {
        if (allocator->lastUse[i] != -1 &&
            allocator->lastUse[i] < currentInstr - 10) {
            // Check if this register contains a spillable variable
            int localIndex = findLocalByRegister(compiler, i);
            if (localIndex != -1) {
                LiveRange* range =
                    &allocator
                         ->ranges[compiler->locals[localIndex].liveRangeIndex];
...
static bool isVariableLoopInvariant(Compiler* compiler, int localIndex) {
    ...
    if (range->firstUse < compiler->loopStart &&
        ...
        return !isVariableModifiedInRange(
            compiler, localIndex, compiler->loopStart, compiler->chunk->count);
    }
...
static bool isVariableModifiedInRange(Compiler* compiler, int localIndex,
                                      int startInstr, int endInstr) {
    // Simplified analysis - in a full implementation, we'd analyze bytecode
    // instructions For now, assume variables are modified if they're assigned
    // to
    ...
    for (int i = startInstr; i < endInstr && i < compiler->chunk->count; i++) {
        ...
        if (instruction == OP_STORE_GLOBAL || instruction == OP_MOVE) {
            // In a full implementation, we'd check if the target matches our
            // local For now, conservatively assume it might be modified
            return true;
        }
    }
...
static void sortSpillCandidatesByPriority(int* candidates, int count,
    ...
    for (int i = 0; i < count - 1; i++) {
        for (int j = 0; j < count - i - 1; j++) {
            if (allocator->lastUse[candidates[j]] >
                allocator->lastUse[candidates[j + 1]]) {
...
static void spillRegister(Compiler* compiler, int regIndex) {
    ...
    // In a full implementation, we'd emit instructions to save the value to
    // memory For now, just track the spill for analysis
...
static bool isUsedInMultiplication(Compiler* compiler, int localIndex,
    ...
    for (int i = loopStart; i < loopEnd && i < compiler->chunk->count; i++) {
        ...
        if (instruction == OP_MUL_I32_R || instruction == OP_MUL_I64_R ||
            instruction == OP_MUL_F64_R) {
...
static bool isRegisterUsedAfterLoop(Compiler* compiler, uint8_t reg,
    ...
    int liveRangeIndex = compiler->locals[localIndex].liveRangeIndex;
    if (liveRangeIndex == -1 || liveRangeIndex >= compiler->regAlloc.count)
        return false;
...
static bool insertInstructionSpace(Compiler* compiler, int offset, int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        chunk->columns[offset + i] =
            chunk->columns[offset > 0 ? offset - 1 : 0];
    }
...
static void updateJumpTargetsAfterInsertion(Compiler* compiler, int insertPos,
    ...
    for (int i = 0; i < compiler->localCount; i++) {
        if (compiler->locals[i].liveRangeIndex >= 0) {
            LiveRange* range =
                &compiler->regAlloc.ranges[compiler->locals[i].liveRangeIndex];
...
__attribute__((unused)) static void promoteLoopInvariantVariables(
    Compiler* compiler, int loopStart, int loopEnd) {
    RegisterAllocator* allocator = &compiler->regAlloc;
    ...
    for (int i = 0; i < allocator->count; i++) {
        ...
        if (!range->isLoopVar && range->start < loopStart &&
            ...
            if (range->isLoopInvariant ||
                ...
                // 1. Assign to preferred registers (lower numbers are often
                // faster)
...
// Implements comprehensive LICM optimization with zero-cost abstractions and
...
bool performLICM(Compiler* compiler, int loopStart, int loopEnd,
                 LoopContext* loopCtx) {
    ...
    // Enhanced implementation that analyzes actual bytecode for hoistable
    // operations
    ...
    if (!analysis.invariantNodes || !analysis.canHoist ||
        !analysis.hoistedRegs || !analysis.originalInstructions) {
    ...
    for (int i = 0; i < analysis.count; i++) {
        analysis.canHoist[i] = canSafelyHoistInstruction(
            &analysis.invariantNodes[i], loopCtx, compiler);
...
bool isLoopInvariant(ASTNode* expr, LoopContext* loopCtx, Compiler* compiler) {
    ...
    switch (expr->type) {
        ...
        case NODE_IDENTIFIER: {
            ...
            if (loopCtx->loopVarIndex >= 0) {
                ...
                if (loopCtx->loopVarIndex < compiler->localCount) {
                    if (strcmp(compiler->locals[loopCtx->loopVarIndex].name,
                               name) == 0) {
        ...
        case NODE_BINARY: {
            ...
            bool leftInvariant =
                isLoopInvariant(expr->binary.left, loopCtx, compiler);
            bool rightInvariant =
                isLoopInvariant(expr->binary.right, loopCtx, compiler);
...
bool canSafelyHoistInstruction(InvariantNode* node, LoopContext* loopCtx,
                               Compiler* compiler) {
...
void hoistInvariantCode(Compiler* compiler, LICMAnalysis* analysis,
                        int preHeaderPos) {
    (void)preHeaderPos;  // Parameter currently unused in simplified
                         // implementation
    Chunk* chunk = compiler->chunk;
    ...
    for (int i = 0; i < analysis->count; i++) {
        ...
        int exprReg __attribute__((unused)) =
            compileExpressionToRegister(expr, compiler);
        ...
        int hoistPos __attribute__((unused)) =
            preHeaderPos >= 0 ? preHeaderPos
                              : 0;  // Simplified for AST-based analysis
    ...
    switch (expr->type) {
        case NODE_IDENTIFIER: {
            ...
            if (loopCtx->loopVarIndex >= 0) {
                ...
                // In a full implementation, we'd compare with
                // locals[loopCtx->loopVarIndex].name
        ...
        case NODE_ASSIGN:
            ...
            if (expr->assign.name && loopCtx->loopVarIndex >= 0) {
                ...
                if (strcmp(expr->assign.name, "i") == 0 ||
                    strcmp(expr->assign.name, "j") == 0 ||
                    strcmp(expr->assign.name, "k") == 0 ||
                    strcmp(expr->assign.name, "counter") == 0) {
                    return true;
                }
            }
...
void collectLoopInvariantExpressions(ASTNode* node, LICMAnalysis* analysis,
    ...
    if (isLoopInvariant(node, loopCtx, compiler)) {
        ...
        if (node->type != NODE_LITERAL && node->type != NODE_IDENTIFIER) {
            ...
            if (analysis->count >= analysis->capacity) {
                int newCapacity =
                    analysis->capacity == 0 ? 8 : analysis->capacity * 2;

                analysis->invariantNodes = realloc(
                    analysis->invariantNodes, sizeof(ASTNode*) * newCapacity);
                analysis->hoistedRegs = realloc(analysis->hoistedRegs,
                                                sizeof(uint8_t) * newCapacity);
                analysis->originalInstructions = realloc(
                    analysis->originalInstructions, sizeof(int) * newCapacity);
                analysis->canHoist =
                    realloc(analysis->canHoist, sizeof(bool) * newCapacity);
    ...
    switch (node->type) {
        case NODE_BINARY:
            collectLoopInvariantExpressions(node->binary.left, analysis,
                                            loopCtx, compiler);
            collectLoopInvariantExpressions(node->binary.right, analysis,
                                            loopCtx, compiler);
            break;

        case NODE_ASSIGN:
            collectLoopInvariantExpressions(node->assign.value, analysis,
                                            loopCtx, compiler);
            break;

        case NODE_IF:
            collectLoopInvariantExpressions(node->ifStmt.condition, analysis,
                                            loopCtx, compiler);
            collectLoopInvariantExpressions(node->ifStmt.thenBranch, analysis,
                                            loopCtx, compiler);
            collectLoopInvariantExpressions(node->ifStmt.elseBranch, analysis,
                                            loopCtx, compiler);
            break;

        case NODE_WHILE:
            collectLoopInvariantExpressions(node->whileStmt.condition, analysis,
                                            loopCtx, compiler);
            collectLoopInvariantExpressions(node->whileStmt.body, analysis,
                                            loopCtx, compiler);
            break;

        case NODE_FOR_RANGE:
            collectLoopInvariantExpressions(node->forRange.start, analysis,
                                            loopCtx, compiler);
            collectLoopInvariantExpressions(node->forRange.end, analysis,
                                            loopCtx, compiler);
            collectLoopInvariantExpressions(node->forRange.step, analysis,
                                            loopCtx, compiler);
            collectLoopInvariantExpressions(node->forRange.body, analysis,
                                            loopCtx, compiler);
            break;

        case NODE_FOR_ITER:
            collectLoopInvariantExpressions(node->forIter.iterable, analysis,
                                            loopCtx, compiler);
            collectLoopInvariantExpressions(node->forIter.body, analysis,
                                            loopCtx, compiler);
            break;

        case NODE_BLOCK:
            for (int i = 0; i < node->block.count; i++) {
                collectLoopInvariantExpressions(node->block.statements[i],
                                                analysis, loopCtx, compiler);
            }
            break;

        case NODE_PRINT:
            for (int i = 0; i < node->print.count; i++) {
                collectLoopInvariantExpressions(node->print.values[i], analysis,
                                                loopCtx, compiler);
            }
            break;
...
__attribute__((unused)) void emitBytes(Compiler* compiler, uint8_t byte1,
                                       uint8_t byte2) {
...
static void hoistInvariantCodeInstruction(Compiler* compiler,
    ...
    for (int i = 0; i < analysis->count; i++) {
        ...
        // Calculate preheader position (just before loop start)
        int hoistPos =
            preHeaderPos >= 0 ? preHeaderPos : node->instructionOffset - 1;
        if (hoistPos < 0) hoistPos = 0;

        // Create space for the hoisted instruction at preheader
        if (insertInstructionSpace(compiler, hoistPos,
                                   4)) {  // 4 bytes for typical arithmetic op
            ...
            if (newOffset + 3 < chunk->count) {
                // Replace original instruction with NOP (OP_MOVE with same
                // src/dst is effectively NOP)
                chunk->code[newOffset] = OP_MOVE;
                chunk->code[newOffset + 1] = node->result;  // destination
                chunk->code[newOffset + 2] =
                    node->result;                // source (already computed)
...
Type* inferExpressionType(Compiler* compiler, ASTNode* expr) {
    ...
    if (compiler->typeInferer) {
        Type* inferredType =
            infer_type((TypeInferer*)compiler->typeInferer, expr);
        if (inferredType) {
            return inferredType;
        }
    }
    ...
    switch (expr->type) {
        ...
        case NODE_IDENTIFIER: {
            int localIndex;
            if (symbol_table_get(&compiler->symbols, expr->identifier.name,
                                 &localIndex)) {
                return get_primitive_type_cached(
                    valueTypeToTypeKind(compiler->locals[localIndex].type));
            }
        ...
        case NODE_BINARY: {
            ...
            if (strcmp(expr->binary.op, "+") == 0 ||
                ...
                strcmp(expr->binary.op, "%") == 0) {
                Type* leftType =
                    inferExpressionType(compiler, expr->binary.left);
                Type* rightType =
                    inferExpressionType(compiler, expr->binary.right);
...
bool resolveVariableType(Compiler* compiler, const char* name,
    ...
    for (int i = compiler->localCount - 1; i >= 0; i--) {
        if (compiler->locals[i].isActive &&
            ...
            if (vtype !=
                VAL_NIL) {  // VAL_NIL indicates no conversion available
...
__attribute__((unused)) static bool getNodeRegisterType(Compiler* compiler,
                                                        ASTNode* node,
                                                        ValueType* outType) {
    ...
    if (strcmp(op, "+") == 0) {
        switch (type) {
            case VAL_I32:
                ...
                break;
            case VAL_I64:
                emitByte(compiler, OP_ADD_I64_TYPED);
                break;
            case VAL_U32:
                emitByte(compiler, OP_ADD_U32_TYPED);
                break;
            case VAL_U64:
                emitByte(compiler, OP_ADD_U64_TYPED);
                break;
            case VAL_F64:
                emitByte(compiler, OP_ADD_F64_TYPED);
                break;
            default:
                emitByte(compiler, OP_ADD_I32_R);
                break;
        ...
        switch (type) {
            case VAL_I32:
                emitByte(compiler, OP_SUB_I32_TYPED);
                break;
            case VAL_I64:
                emitByte(compiler, OP_SUB_I64_TYPED);
                break;
            case VAL_U32:
                emitByte(compiler, OP_SUB_U32_TYPED);
                break;
            case VAL_U64:
                emitByte(compiler, OP_SUB_U64_TYPED);
                break;
            case VAL_F64:
                emitByte(compiler, OP_SUB_F64_TYPED);
                break;
            default:
                emitByte(compiler, OP_SUB_I32_R);
                break;
        ...
        switch (type) {
            case VAL_I32:
                emitByte(compiler, OP_MUL_I32_TYPED);
                break;
            case VAL_I64:
                emitByte(compiler, OP_MUL_I64_TYPED);
                break;
            case VAL_U32:
                emitByte(compiler, OP_MUL_U32_TYPED);
                break;
            case VAL_U64:
                emitByte(compiler, OP_MUL_U64_TYPED);
                break;
            case VAL_F64:
                emitByte(compiler, OP_MUL_F64_TYPED);
                break;
            default:
                emitByte(compiler, OP_MUL_I32_R);
                break;
        ...
        switch (type) {
            case VAL_I32:
                emitByte(compiler, OP_LT_I32_TYPED);
                break;
            case VAL_I64:
                emitByte(compiler, OP_LT_I64_TYPED);
                break;
            case VAL_U32:
                emitByte(compiler, OP_LT_U32_TYPED);
                break;
            case VAL_U64:
                emitByte(compiler, OP_LT_U64_TYPED);
                break;
            case VAL_F64:
                emitByte(compiler, OP_LT_F64_TYPED);
                break;
            default:
                emitByte(compiler, OP_LT_I32_R);
                break;
