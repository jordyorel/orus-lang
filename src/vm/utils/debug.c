...
//  ---------------------------------------------------------------------------
//  File: src/vm/utils/debug.c
//  Author: Jordy Orel KONDA
//  Copyright (c) 2025 Jordy Orel KONDA
//  License: MIT License (see LICENSE file in the project root)
//  Description: Implements runtime debugging utilities and bytecode inspection helpers.
//
...
int disassembleInstruction(Chunk* chunk, int offset) {
    ...
    switch (instruction) {
        ...
        case OP_ASSERT_EQ_R: {
            ...
            uint8_t actual_reg = chunk->code[offset + 3];
            uint8_t expected_reg = chunk->code[offset + 4];
            printf("%-16s dst=R%d, label=R%d, actual=R%d, expected=R%d\n",
                   "ASSERT_EQ", dst, label_reg, actual_reg, expected_reg);
            return offset + 5;
        }
